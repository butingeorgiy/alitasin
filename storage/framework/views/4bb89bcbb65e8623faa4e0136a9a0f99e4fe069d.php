<a href="<?php echo e(route('tour', $tour->id)); ?>" target="<?php echo e($isAdmin ? '_blank' : '_self'); ?>"
   class="tour-card flex flex-col shadow rounded-md">
    <div class="flex justify-end items-start p-3 bg-center bg-cover bg-no-repeat bg-gray-50 rounded-md"
         style="height: 180px; background-image: url(<?php echo e($tour->image); ?>)">
        <?php if($isAdmin): ?>
            <div class="move-to-update-page flex justify-center items-center w-8 h-8 mr-2 bg-white rounded-full cursor-pointer"
                 data-tour-id="<?php echo e($tour->id); ?>">
            ...
            <div class="delete-tour-button flex justify-center items-center w-8 h-8 bg-white rounded-full cursor-pointer"
                ...
                <svg class="min-w-4 min-h-4 w-4 h-4" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M2.625 5.25H4.375H18.375" stroke="#FF3D3D" stroke-width="2" stroke-linecap="round"
                    ...
                    <path d="M12.25 9.625V14.875" stroke="#FF3D3D" stroke-width="2" stroke-linecap="round"
                          stroke-linejoin="round"/>
                    <path d="M8.75 9.625V14.875" stroke="#FF3D3D" stroke-width="2" stroke-linecap="round"
                          stroke-linejoin="round"/>
                </svg>
            </div>
        <?php else: ?>
            <div class="flex justify-center items-center w-8 h-8 bg-white rounded-full cursor-pointer">
                <div class="toggle-favorite-button min-w-5 min-h-5 w-5 h-5 bg-contain bg-center bg-no-repeat"
                     data-tour-id="<?php echo e($tour->id); ?>"
                     style="background-image: url(<?php echo e(asset('images/' . ($tour->is_favorite === '1' ? 'active-' : '') . 'heart-icon.svg')); ?>)"></div>
            </div>
        <?php endif; ?>
    </div>
    <div class="flex flex-col px-4 pb-2 h-full" style="flex: 1">
        <div class="flex items-center py-3 text-xs 2xl:text-sm text-blue font-semibold">
            <div class="flex items-center mr-4">
                ...
                <span><?php echo e($tour->type->name); ?></span>
            </div>
            ...
            <?php if($tour->duration): ?>
                <div class="flex items-center">
                    ...
                    <span><?php echo e($tour->duration); ?></span>
                </div>
            <?php endif; ?>
        </div>

        <p class="mb-2 text-black font-bold leading-5"><?php echo e($tour->title[App::getLocale()]); ?></p>
        <p class="mb-4 text-sm text-gray-600 leading-5"><?php echo e(\Illuminate\Support\Str::limit(strip_tags($tour->description[App::getLocale()]))); ?></p>

        <div class="flex items-end" style="flex: 1">
            <p class="mr-auto text-3xl text-black font-bold">$<?php echo e($tour->price); ?></p>
            <div class="flex flex-col items-end pb-0.5">
                <div class="flex mb-1">
                    <div class="min-w-4 min-w-4 w-4 h-4 mr-1 bg-contain bg-no-repeat bg-center"
                         style="background-image: url(<?php echo e(asset('images/active-start.svg')); ?>)"></div>
                    <div class="min-w-4 min-w-4 w-4 h-4 mr-1 bg-contain bg-no-repeat bg-center"
                         style="background-image: url(<?php echo e(asset('images/active-start.svg')); ?>)"></div>
                    <div class="min-w-4 min-w-4 w-4 h-4 mr-1 bg-contain bg-no-repeat bg-center"
                         style="background-image: url(<?php echo e(asset('images/active-start.svg')); ?>)"></div>
                ...
                <span class="text-sm text-gray-600 font-medium hover:underline"><?php echo e(__('short-phrases.reviews')); ?>
...
<?php /**PATH /Users/butingeorgiy/Workspace/Dev/www-root/alitasin/resources/views/components/tours/tour-card.blade.php ENDPATH**/ ?>