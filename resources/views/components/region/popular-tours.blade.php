@if(count($popularTours) > 0)
    <section id="popularToursSection" class="mt-4 mb-10 pb-4 border-b border-gray-200">
            ...
            <div class="swiper-container -ml-1">
                <div class="swiper-wrapper pl-1 py-2">
                    @foreach($popularTours as $tour)
                        <div class="swiper-slide flex flex-col relative w-72 h-auto p-3 bg-cover bg-center bg-no-repeat shadow rounded-md"
                            ...
                            <p class="z-10 mt-auto text-white font-medium leading-5">{{ $tour->title[App::getLocale()] }}</p>
                        </div>
                    @endforeach
                </div>
            </div>
