...
<head>
    ...
    <title>{{ __('page-titles.edit-vehicle') }}</title>
</head>
        ...
        <form id="editVehicleForm" class="container mx-auto px-5">
            <div>
                ...
                <div class="grid grid-cols-5 gap-5">
                    ...
                    $images = $vehicle->images->sortByDesc('is_main')->values();
                    ?>
                    @for($i = 1; $i <= 5; $i++)
                        @if(isset($images[$i-1]))
                            <label class="image-item filled"
                                   style="background-image: url({{ route('get-image', ['dir' => 'vehicle_pictures', 'file' => $images[$i-1]->image]) }})"
                                   data-image-id="{{ $images[$i-1]->id }}">
                                @if($i !== 1)
                                    <div
                                        class="make-image-main-button absolute -bottom-6 text-sm text-gray-600 font-light hover:underline">
            ...
            <div class="mt-12">
                ...
                <select name="show_at_index_page"
                        class="w-full mb-5 px-4 py-3 text-sm text-gray-400 placeholder-gray-400 bg-white shadow rounded-md cursor-pointer">
                    <option value="0" {{ $vehicle->show_at_index_page === '0' ? 'selected' : '' }}>{{ __('short-phrases.no') }}</option>
                    <option value="1" {{ $vehicle->show_at_index_page === '1' ? 'selected' : '' }}>{{ __('short-phrases.yes') }}</option>
                </select>
            ...
            <div class="grid grid-cols-2 gap-5 mt-12">
                <div class="flex flex-col">
                    <div class="flex items-center mb-2">
                        ...
                        <span
                            class="open-param-popup-button text-sm text-blue cursor-pointer whitespace-nowrap hover:underline"
                            data-is-include="1">{{ __('buttons.add') }}</span>
                    </div>
                    <div class="params-container" data-params="{{ json_encode($vehicle->params) }}">
                        @forelse($vehicle->params as $param)
...
{{--                                    {{ dd($param) }}--}}
                                    ...
                                    <p class="text-sm text-gray-400 italic">Ru:&nbsp;&nbsp;{{ $param->getOriginal('pivot_ru_value') ?: 'Ничего не указано' }}</p>
                                    <p class="text-sm text-gray-400 italic">Tr:&nbsp;&nbsp;{{ $param->getOriginal('pivot_tr_value') ?: 'Ничего не указано' }}</p>
                                    <p class="text-sm text-gray-400 italic">Ua:&nbsp;&nbsp;{{ $param->getOriginal('pivot_ua_value') ?: 'Ничего не указано' }}</p>
                                </div>
                            </div>
                        @empty
            ...
            <div class="save-vehicle-button mt-6 bg-green-600">
