...
<head>
    ...
    <title>{{ __('page-titles.create-property') }}</title>
</head>
<body class="bg-gray-50">
    @include('components.general.header')

    <div class="mt-12 pb-12">
        <form id="createPropertyForm" class="container mx-auto px-5">
            <div>
                    ...
                    @for($i = 1; $i <= 5; $i++)
                        ...
                        <span class="empty flex flex-col items-center">
                            <svg class="w-12 h-12 mb-3" viewBox="0 0 83 83" fill="none"
                                ...
                                <path d="M70.375 33.25C69.281 33.25 68.2318 33.6846 67.4582 34.4582C66.6846 35.2318 66.25 36.281
            ...
            <div class="mt-12">
                ...
                <select name="type_id"
                    ...
                    @foreach(App\Models\PropertyType::all() as $type)
                        <option value="{{ $type->id }}">{{ $type->getLocaleName() }}</option>
                    @endforeach
                </select>
                ...
                <select name="cost_unit_id"
                    ...
                    @foreach(App\Models\CostUnit::all() as $unit)
                        <option value="{{ $unit->id }}">{{ $unit->getLocaleName() }}</option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-2 gap-5 mt-12">
                <div class="flex flex-col">
                    <div class="flex items-center mb-2">
                        <p class="mr-4 font-semibold">{{ __('short-phrases.parameters') }} ({{ __('short-phrases.unnecessary') }})</p>
                        <span class="open-param-popup-button text-sm text-blue cursor-pointer whitespace-nowrap hover:underline" data-is-include="1">{{ __('buttons.add') }}</span>
                    </div>
